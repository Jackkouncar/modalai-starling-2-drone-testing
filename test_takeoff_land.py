...
class TestTakeoffLand(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f"Safety radius: {safety_max_horizontal_error():.2f} m horizontal, "
    ...
    def outside_safety_box(self):
        if self.home_x is None:
            return "home was not captured yet"
        ...
        max_horizontal_error = safety_max_horizontal_error()

        if horizontal_error > max_horizontal_error:
            return (
                f"horizontal drift {horizontal_error:.2f} m exceeded "
                f"{max_horizontal_error:.2f} m"
            )
    ...
    def timer_cb(self):
        ...
        if self.state in (State.TAKEOFF, State.HOVER, State.DESCEND):
            safety_reason = self.outside_safety_box()
            if safety_reason is None:
                self.safety_violation_started_at = None
            else:
                if self.safety_violation_started_at is None:
                    self.safety_violation_started_at = time.time()
                    self.get_logger().warn(f"Safety warning: {safety_reason}")
                elif time.time() - self.safety_violation_started_at > SAFETY_VIOLATION_GRACE_SECONDS:
                    self.abort_to_land(f"Safety box exceeded: {safety_reason}. Commanding land.")
                    return
